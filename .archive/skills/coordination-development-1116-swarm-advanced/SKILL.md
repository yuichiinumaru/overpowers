...
This skill includes helper scripts to generate complex configurations:

- **scripts/swarm_init_helper.py**: Generates swarm initialization JSON based on topology (mesh, hierarchical, star, ring).
- **scripts/spawn_agents_helper.py**: Generates agent spawn configurations for research, development, or testing teams.

Usage:
```bash
# Generate mesh topology config
python3 scripts/swarm_init_helper.py mesh --agents 6

# Generate research team agent configs
python3 scripts/spawn_agents_helper.py research
```
