# Overpowers Release Notes

## v4.0.3 (2025-12-26)

### Improvements

**Strengthened using-Overpowers skill for explicit skill requests**
...
The underlying skills (`Overpowers:brainstorming`, `Overpowers:executing-plans`, `Overpowers:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-Overpowers` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-Overpowers`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `Overpowers:brainstorming`)
...
New prompt templates in `skills/subagent_driven_development/`:
...
**Skill priority in using-Overpowers**
...
- **Rationalizations table** in using-Overpowers - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-Overpowers is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > Overpowers
...
- Optimized Overpowers bootstrap to eliminate redundant skill execution. The `using-Overpowers` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-Overpowers` manually despite already having the content from session start.
...
- Added unified `Overpowers-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `Overpowers:skill-name` for Overpowers skills, `skill-name` for personal
...
- Updated terminology: "Overpowers skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/Overpowers-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/Overpowers-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core Overpowers functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-Overpowers skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-Overpowers/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-Overpowers skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-Overpowers/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `Overpowers:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `Overpowers:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code_reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting_code_review/SKILL.md` - References to `Overpowers:code-reviewer`
- Updated: `skills/subagent_driven_development/SKILL.md` - References to `Overpowers:code-reviewer`
...
- All internal skill references now use `Overpowers:` namespace prefix
- Updated format: `Overpowers:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/Overpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
- Added `skills/writing_skills/anthropic-best-practices.md` - Official Anthropic skill authoring guide
- Referenced in writing_skills SKILL.md for comprehensive guidance
...
# Overpowers v2.0.0 Release Notes

## Overview

Overpowers v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([obra/Overpowers-skills](https://github.com/obra/Overpowers-skills)). This transforms Overpowers from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [obra/Overpowers-skills](https://github.com/obra/Overpowers-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/Overpowers/skills/`
...
1. Your old `~/.config/Overpowers/.git` will be backed up to `~/.config/Overpowers/.git.bak`
2. Old skills will be backed up to `~/.config/Overpowers/skills.bak`
3. Fresh clone of obra/Overpowers-skills will be created at `~/.config/Overpowers/skills/`

### Removed Features

- **Personal Overpowers overlay system** - Replaced with git branch workflow
- **setup-personal-Overpowers hook** - Replaced by initialize-skills.sh
...
- Clones obra/Overpowers-skills on first run
...
- `Overpowers_SKILLS_ROOT` set to `~/.config/Overpowers/skills`
...
- Removed obsolete setup-personal-Overpowers call from session-start
...
- Prominent link to Overpowers-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in obra/Overpowers-skills
- `scripts/` directory - Now in obra/Overpowers-skills/skills/using-skills/
- `hooks/setup-personal-Overpowers.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/Overpowers/skills
- `commands/brainstorm.md` - Updated paths to Overpowers_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to Overpowers_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to Overpowers_SKILLS_ROOT
...
- PR #2: Personal Overpowers overlay system (later replaced)
...
```bash
# In Claude Code
/plugin marketplace add obra/Overpowers-marketplace
/plugin install Overpowers@Overpowers-marketplace
```
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/Overpowers/skills ~/Overpowers-skills-backup
   ```

2. **Update the plugin:**
   ```bash
   /plugin update Overpowers
   ```
...
- Skills repository is now at https://github.com/obra/Overpowers-skills
- Fork → Branch → PR workflow
- See skills/meta/writing_skills/SKILL.md for TDD approach to documentation
...
**Full Changelog:** https://github.com/obra/Overpowers/compare/dd013f6...main
**Skills Repository:** https://github.com/obra/Overpowers-skills
**Issues:** https://github.com/obra/Overpowers/issues
