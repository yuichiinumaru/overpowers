...
## [2026-03-02] - Fix Unresolvable MCP Environment Variables

### Fixed
- Fixed unresolvable `{env:VAR}` variables generated by `install-mcps.sh` for `serena`, `desktop-commander`, and `semgrep` by defining correct template values in `.env.example` and standardizing command patterns in `opencode-example.json`.
