...
if [[ "${FAST_MODE}" == "1" ]]; then
    ...
    if command -v gum >/dev/null 2>&1; then
        ...
        read -rp "Deploy to detected only (1) or all 9 (2)? [1/2]: " p_choice
        [[ "$p_choice" == "1" ]] && SELECTED_PLATFORMS=("${AUTO_DETECTED[@]}") || SELECTED_PLATFORMS=("OpenCode" "GeminiCLI" "Antigravity" "KiloCode" "Cursor" "Windsurf" "CodexCLI" "ClaudeCode" "Factory")
    fi
fi
...
for p in "${SELECTED_PLATFORMS[@]}"; do
    case "$p" in
        ...
        GeminiCLI) bash "${SCRIPT_DIR}/scripts/deploy-to-gemini-cli.sh" ;;
