...
test_connections() {
    ...
    if [[ -f "../configs/hetzner-config.json" ]]; then
        ...
        if ../.agent/skills/hetzner-integration/scripts/hetzner-helper.sh accounts 2>/dev/null; then
    ...
    if [[ -f "../configs/spaceship-config.json" ]]; then
        ...
        if ../.agent/skills/spaceship-integration/scripts/spaceship-helper.sh accounts 2>/dev/null; then
    ...
    if [[ -f "../configs/git-platforms-config.json" ]]; then
        ...
        if ../.agent/skills/github-release-management/scripts/git-platforms-helper.sh platforms 2>/dev/null; then
