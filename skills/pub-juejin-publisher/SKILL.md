---
name: pub-juejin-publisher
description: Automatically publish Markdown articles to Juejin platform using official API with support for categories, tags, and covers.
tags: [publishing, juejin, article, blog, automation]
version: 1.0.0
---
...
```bash
# 在 skill 根目录创建配置文件
cp skills/pub-juejin-publisher/juejin.env.example \
   skills/pub-juejin-publisher/juejin.env

# 编辑配置文件，填入你的 Cookie
nano skills/pub-juejin-publisher/juejin.env
```
...
```bash
# 赋予执行权限
chmod +x skills/pub-juejin-publisher/scripts/publish.py

# 基本用法（使用默认分类和标签）
python3 skills/pub-juejin-publisher/scripts/publish.py article.md
...
# 仅创建草稿，不发布
python3 skills/pub-juejin-publisher/scripts/publish.py article.md --draft-only
```
...
> ```bash
> python3 skills/pub-juejin-publisher/scripts/query_tags.py "关键词"
> ```
