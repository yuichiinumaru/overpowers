// 查询用户详情 (安全版本)
// 用法: ts-node scripts/get-user-safe.ts <userId> [--debug]
// 凭证通过环境变量 DINGTALK_APP_KEY 和 DINGTALK_APP_SECRET 提供

import * as https from 'https';
import { default as dingtalkOauth2_1_0, GetAccessTokenRequest } from '@alicloud/dingtalk/oauth2_1_0';
  ...
  if (!appKey || !appSecret) {
    console.error(JSON.stringify({
      ...
      error: {
        ...
        usage: 'export DINGTALK_APP_KEY=your-app-key && export DINGTALK_APP_SECRET=your-app-secret && ts-node scripts/get-user-safe.ts <userId> [--debug]'
      }
    }, null, 2));
    process.exit(1);
  }
  ...
  if (filteredArgs.length < 1) {
    console.error(JSON.stringify({
      ...
      error: {
        ...
        usage: 'ts-node scripts/get-user-safe.ts <userId> [--debug]'
      }
    }, null, 2));
    process.exit(1);
  }
