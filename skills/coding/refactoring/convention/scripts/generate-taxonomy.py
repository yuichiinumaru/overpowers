...
def infer_category_from_content(skill_path: Path, original_name: str) -> Tuple[str, str, str]:
    """
    Infer L1-L2-L3 baseado em análise semântica + prefixo original.
    
    Estratégia híbrida:
    1. Se original já tem prefixo bom (ai-llm-*, dev-code-*, etc), PRESERVA
    2. Senão, infer via keywords do SKILL.md
    """
    skill_md = skill_path / "SKILL.md"
    
    # Tenta extrair L1-L2 do nome original
    parts = original_name.split('-')
    if len(parts) >= 2:
        prefix_l1 = parts[0]
        prefix_l2 = parts[1] if len(parts) > 1 else None
        
        # Prefixos conhecidos que devemos preservar
        KNOWN_L1 = {'ai', 'dev', 'data', 'content', 'tools', 'agent', 'workflows', 'biz', 'ops', 'sec', 'sci', 'design', 'growth', 'infra', 'frontend', 'backend', 'web', 'mobile', 'cloud', 'infra-ops', 'design-ux', 'content-media', 'data-sci', 'dev-code', 'ai-llm', 'growth-biz'}
        
        if prefix_l1 in KNOWN_L1 or (prefix_l1 + '-' + prefix_l2) in KNOWN_L1:
            # Preserva prefixo original
            l1 = prefix_l1
            l2 = prefix_l2 if prefix_l2 and prefix_l2.isdigit() == False else 'default'
            l3 = 'default'  # Será usado para chunking
            return l1, l2, l3
    
    # Fallback: infer via conteúdo
    if not skill_md.exists():
        return 'misc', 'general', 'default'
    ...
    l2_best = max(l2_scores.items(), key=lambda x: x[1])[0] if l2_scores else 'default'
    
    return l1_best, l2_best, 'default'
...
def generate_taxonomy(skills_dir: Path, levels: int = 3, use_llm: bool = False) -> Dict:
    ...
    for i, skill in enumerate(skills):
        ...
        if parsed.get('valid') and parsed['levels'] >= levels:
            ...
            # Precisa inferir (estratégia híbrida: preserva prefixo se bom)
            l1, l2, l3 = infer_category_from_content(skill_path, skill_name)
