...
This skill includes a utility script to automate common Terraform operations:

- **scripts/tf_tools.sh**: A multi-command wrapper that works with both `terraform` and `tofu`.

Usage:
```bash
# Format files recursively
./scripts/tf_tools.sh fmt ./modules

# Initialize (without backend) and validate
./scripts/tf_tools.sh validate ./infrastructure/prod

# Generate documentation for a module (requires terraform-docs)
./scripts/tf_tools.sh docs ./modules/vpc

# Run security scanning (requires tfsec or checkov)
./scripts/tf_tools.sh sec ./modules
```
