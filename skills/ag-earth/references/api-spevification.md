...
### 模式与准入
- 默认为本地模式：不进行外部请求。
- 外部模式需显式启用：仅在完成合规检查与用户授权后才允许请求。
- 发送参数须**脱敏与最小化**：不得包含姓名、电话、精确住址、账号、证件号等敏感信息；不发送原始聊天记录，使用摘要化的任务查询。
...
- 默认为本地模式：不进行外部请求。
- 外部模式需显式启用：仅在完成合规检查与用户授权后才允许请求。
- 发送参数须**脱敏与最小化**：不得包含敏感个人信息；不发送原始聊天记录，使用结构化必要字段。
...
1. **推荐调用前**：确保 query 足够具体，并完成脱敏与最小化
