...
description: Guia e padrões de arquitetura para a construção de agentes de IA autônomos utilizando um Agent Development Kit (ADK) ou frameworks similares (ex: LangChain, CrewAI). Foca no ciclo ReAct (Reason + Act) e na integração de ferramentas (Tools) e Memória.
