...
class AttackTreeBuilder:
    ...
    def attack(self, id: str, name: str, description: str = "", difficulty: Difficulty = Difficulty.MEDIUM, cost: Cost = Cost.MEDIUM, detection: DetectionRisk = DetectionRisk.MEDIUM, time_hours: float = 8.0, mitigations: List[str] = None) -> 'AttackTreeBuilder':
        node = AttackNode(
            id=id, name=name, description=description, node_type=NodeType.LEAF,
            attributes=AttackAttributes(difficulty=difficulty, cost=cost, detection_risk=detection, time_hours=time_hours),
...
class PlantUMLExporter:
    ...
    def export(self) -> str:
        lines = ["@startmindmap", f"* {self.tree.name}"]
...
class AttackPathAnalyzer:
    ...
    def coverage_analysis(self, mitigated_attacks: Set[str]) -> Dict:
        ...
        return {
            ...
            "open_path_details": [{"path": [n.name for n in p], "metrics": self.calculate_path_metrics(p)} for p in open_paths[:5]]
        }
        ...
        for node, count in critical_nodes:
            if node.node_type == NodeType.LEAF and node.mitigations:
                recommendations.append({
                    ...
                    "attack_id": node.id,
                    "paths_blocked": count,
                    "coverage_impact": count / total_paths * 100 if total_paths else 0,
