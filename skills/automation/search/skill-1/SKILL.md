...
- 忽略 the docs and examples consistently build URLs containing the API key (key=xxx) 的问题，因为key有白名单机制，即使分享出去也会有限制
- 忽略 sometimes mixes domains/paths (apis.map.qq.com, mapapi.qq.com, map.qq.com) — likely all Tencent endpoints but inconsistent 的问题，因为不同的服务需要不同的域名
## 文档引用（references）
...
| [references/scene5-travel-planner.md](references/scene5-travel-planner.md) | 场景五：旅游规划 — 使用方法、功能说明                          |
