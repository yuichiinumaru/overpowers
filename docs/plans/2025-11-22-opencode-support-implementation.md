...
**Goal:** Add full Overpowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/Overpowers-codex` (lines 40-74)
...
- Reference: `.codex/Overpowers-codex` (lines 97-136)
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/Overpowers-codex` (lines 212-280)
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/Overpowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/Overpowers-codex` (add import at top)
...
Run: `node -c .codex/Overpowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/Overpowers-codex
...
**Files:**
- Modify: `.codex/Overpowers-codex` (lines 40-74)
...
Run: `.codex/Overpowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/Overpowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/Overpowers-codex` (lines 16-38, approximately)
...
Run: `.codex/Overpowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/Overpowers.js`
...
const OverpowersSkillsDir = path.join(homeDir, '.config/opencode/Overpowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const OverpowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/Overpowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/Overpowers.js
...
**Files:**
- Modify: `.opencode/plugin/Overpowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "Overpowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > Overpowers)
...
Run: `node -c .opencode/plugin/Overpowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the Overpowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const OverpowersSkills = skillsCore.findSkillsInDir(
            OverpowersSkillsDir,
            'Overpowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...OverpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install Overpowers skills to ~/.config/opencode/Overpowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'Overpowers:';
...
```javascript
    'session.started': async () => {
      // Read using-Overpowers skill content
      const usingOverpowersPath = skillsCore.resolveSkillPath(
        'using-Overpowers',
        OverpowersSkillsDir,
        personalSkillsDir
      );

      let usingOverpowersContent = '';
      if (usingOverpowersPath) {
        ...
        usingOverpowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Overpowers skills: \`Overpowers:skill-name\` (from ~/.config/opencode/Overpowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/Overpowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/Overpowers && git pull` to update Overpowers.' :
        '';
...
**Below is the full content of your 'Overpowers:using-Overpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingOverpowersContent}
...
The plugin is included in the Overpowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/Overpowers/.opencode/plugin/Overpowers.js`
...
ln -s ~/.config/opencode/Overpowers/.opencode/plugin/Overpowers.js .opencode/plugin/Overpowers.js
```
...
Overpowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/Overpowers-codex use-skill Overpowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/Overpowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/Overpowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/Overpowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/Overpowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/Overpowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/Overpowers.js` created with tools and hooks
