...
Add full Overpowers support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port Overpowers to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/Overpowers-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/Overpowers.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/Overpowers/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/Overpowers-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "Overpowers:brainstorming")')
  }),
...
1. **Inject using-Overpowers content**
   - Full content of the using-Overpowers skill
...
export const OverpowersPlugin = async ({ client, directory, $ }) => {
  const OverpowersDir = path.join(process.env.HOME, '.config/opencode/Overpowers');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingOverpowers = await readSkill('using-Overpowers');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingOverpowers}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── Overpowers-codex        # UPDATED: Use skills-core
│   ├── Overpowers-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── Overpowers.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/Overpowers-codex`
...
2. Update `.codex/Overpowers-codex` to use shared core
...
1. Create `.opencode/plugin/Overpowers.js`
