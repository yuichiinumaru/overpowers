# Overpowers for OpenCode

Complete guide for using Overpowers with [OpenCode.ai](https://opencode.ai).
...
ln -sf ~/.config/opencode/Overpowers/.opencode/plugin/Overpowers.js .opencode/plugin/Overpowers.js
```

#### 3. Restart OpenCode

Restart OpenCode to load the plugin. Overpowers will automatically activate.
...
3. **Overpowers skills** (`~/.config/opencode/Overpowers/skills/`)

You can force resolution to a specific level:
- `project:skill-name` - Force project skill
- `skill-name` - Search project → personal → Overpowers
- `Overpowers:skill-name` - Force Overpowers skill

## Features

### Automatic Context Injection

The plugin automatically injects Overpowers context via the chat.message hook on every session. No manual configuration needed.
...
The plugin listens for `session.compacted` events and automatically re-injects the core Overpowers bootstrap to maintain functionality after context compaction.
...
**Location:** `~/.config/opencode/Overpowers/.opencode/plugin/Overpowers.js`
...
**Location:** `~/.config/opencode/Overpowers/lib/skills-core.js`
...
2. Check symlink: `ls -l ~/.config/opencode/plugin/Overpowers.js`
3. Check OpenCode logs: `opencode run "test" --print-logs --log-level DEBUG`
4. Look for: `service=plugin path=file:///.../Overpowers.js loading plugin`

### Skills not found

1. Verify skills directory: `ls ~/.config/opencode/Overpowers/skills`
...
3. Test plugin manually: `node --input-type=module -e "import('file://~/.config/opencode/plugin/Overpowers.js').then(m => console.log(Object.keys(m)))"`

### Context not injecting

1. Check if chat.message hook is working
2. Verify using-Overpowers skill exists
