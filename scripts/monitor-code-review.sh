...
run_qlty_analysis() {
    ...
    if bash "$REPO_ROOT/.agent/skills/code-auditor/scripts/qlty-cli.sh" check 5 > "$REPO_ROOT/.agent/tmp/qlty-results.txt" 2>&1; then
        local issues
        ...
        if bash "$REPO_ROOT/.agent/skills/code-auditor/scripts/qlty-cli.sh" fmt --all > "$REPO_ROOT/.agent/tmp/qlty-fmt.txt" 2>&1; then
...
run_codacy_analysis() {
    ...
    # Run in background
    bash "$REPO_ROOT/.agent/skills/code-auditor/scripts/codacy-cli.sh" analyze --fix > "$log_file" 2>&1 &
    local pid=$!
