#!/usr/bin/env bash
# =============================================================================
# uninstall-from-codex.sh - Remove Overpowers from Codex CLI
# =============================================================================
# Reverses all changes made by deploy-to-codex.sh
# Handles TOML config (config.toml)
# =============================================================================
...
# --- Configuration ---
PLATFORM_NAME="Codex"
PLATFORM_DIR="${HOME}/.codex"
BACKUP_PLATFORM="codex"

# Symlinks created by deploy-to-codex.sh
declare -A SYMLINKS=(
    ["skills"]="skills"
    ["AGENTS.MD"]="AGENTS.MD"
)

# Config files to validate (TOML format)
CONFIG_FILES=(
    "${PLATFORM_DIR}/config.toml"
)
...
# Validate TOML configs after removal
echo ""
...
for config_file in "${CONFIG_FILES[@]}"; do
    if [[ -f "${config_file}" ]]; then
        if validate_toml "${config_file}"; then
            : # Validation passed
        else
            ERRORS+=("Invalid TOML config: ${config_file}")
        fi
    else
