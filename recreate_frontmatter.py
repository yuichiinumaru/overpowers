#!/usr/bin/env python3
"""Recreate proper YAML frontmatter for extracted skills."""
...
def fix_frontmatter(skill_name: str, data: dict):
    ...
    # Find where the actual content starts (after all frontmatter and empty lines)
    content_start = 0
    in_frontmatter = False
    found_closing_dash = False
    
    for i, line in enumerate(lines):
        stripped = line.strip()
        
        # Skip initial frontmatter
        if stripped == '---':
            if not in_frontmatter:
                in_frontmatter = True
            elif in_frontmatter and not found_closing_dash:
                found_closing_dash = True
                content_start = i + 1
                break
        elif found_closing_dash:
            break
        
        # Skip name/description/tags/version/category lines
        if in_frontmatter:
            if any(stripped.startswith(k) for k in ['name:', 'description:', 'tags:', 'version:', 'category:', '  -']):
                continue
    ...
    # Skip any stray name/description lines that might have slipped through
...
if __name__ == "__main__":
    print("Recreating YAML frontmatter for extracted skills...\n")
