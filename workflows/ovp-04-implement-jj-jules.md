...
1. **Context Initialization (Explicit Memory Read)**:
   - Read `.agents/continuity-<agent-name>.md` to align with the current session focus and active branches.
...
5. **Memory Synchronization (Explicit Memory Update)**:
   - Update `.agents/continuity-<agent-name>.md` to reflect the progress made or the completion of the task.
