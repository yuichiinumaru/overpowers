...
1. **Context Initialization (Explicit Memory Read)**: 
   - Read `.agents/continuity-<agent-name>.md` and check `.agents/memories/` to ensure the task sequence is consistent with other parallel work.

2. **Read Context**: Analyze the unified SDD dual/triplet: 
   - `docs/tasks/nnnn-type-subtype-[name]-feature-plan.md`
   - `docs/tasks/nnnn-type-subtype-[name]-technical-design.md`

3. **Breakdown**: Decompose the feature into a detailed, granular, and sequential checklist of sub-tasks for actual implementation. Identify tasks that can be done in parallel (`01-9`) vs sequence blockers (`0`).
...
4. **Create Master Task**: 
   ...
   - Populate the file with the sub-tasks checklist mapped out in step 3.
...
6. **Memory Synchronization (Explicit Memory Update)**: 
   - Update the Session Ledger (`.agents/continuity-<agent-name>.md`) to reflect the new active task and implementation plan.
