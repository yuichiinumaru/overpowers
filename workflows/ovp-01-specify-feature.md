...
1. **Context Initialization (Explicit Memory Read)**: 
   ...
   - Review any existing project context or parent plans that this feature must inherit.

2. **Understand Input**: Review the raw idea or the provided document in `docs/tasks/planning/`.

3. **Interview User**: Ask clarifying questions to extract:
...
5. **Memory Synchronization (Explicit Memory Update)**: 
   - Update `.agents/continuity-<agent-name>.md` to record the creation of this new feature plan.
   - Persist any major discovery made during the interview to `.agents/memories/` using Serena.
