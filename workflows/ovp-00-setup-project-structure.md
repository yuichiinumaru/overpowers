...
Apply the **Overpowers AGENTS.md template** (`templates/rules/AGENTS.md`) to the current project as the single source of truth for agent governance.
...
2. **Merge existing rules with the template.**
   ...
   - Identify sections that are **universal rules** from the template (archive protocol, changelog protocol, operational laws, FILENAME CONVENTIONS, TASK SYSTEM, engineering standards, VCS guardrails, security boundaries).
   - **Crucially:** Inform the user that the Filename & Naming Conventions (`nnnn-type-subtype-names.md`) and the Task Management Protocols (`docs/tasklist.md` and `docs/tasks/`) are **MANDATORY** for any project leveraging this setup. They must be included and enforced.
   - For each universal rule:
     - If the existing file has NO equivalent, **add it** from the template.
...
4. **Propagate dependent templates.**
   ...
   - Copy task templates from `$OVERPOWERS_PATH/templates/tasks/` if they don't already exist:
     - `000-template.md`
     - `000-template-feature-plan.md`
     - `000-template-technical-design.md`
   - Ensure `.agents/` directory exists with subdirectories:
     - `.agents/memories/`
     - `.agents/thoughts/`
   - Create `.agents/continuity-<agent-name>.md` if it doesn't exist.
   - Create `CHANGELOG.md` in the project root if it doesn't exist, initializing it with the standard format.
   - Create `.archive/` directory if it doesn't exist.
   - Ensure `scripts/utils/` directory exists.
   - Copy `$OVERPOWERS_PATH/scripts/utils/jj-commit-push.sh` to `scripts/utils/jj-commit-push.sh` and make it executable (`chmod +x`).
