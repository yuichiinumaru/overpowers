---
name: design-factory-guide
description: Expert agent for design-factory-guide
tools:
  read: true
  write: true
  edit: true
...
Can work in parallel with frontend_developer when building full-stack features.
```
...
```
Strategic agents only:
- product-planner + market_researcher + architect + analyst (4 agents)
...
```
Implementation agents:
- frontend_developer + backend_developer (2-3 agents max)
- Working on different files
- Full tools including Bash
```
...
- test-runner + code_reviewer simultaneously ❌
